...
class Dataset(TorchDataset):
    ...
    def __init__(self, label, rel_types, entity_types, neg_entity_count,
        ...
        self._mode = Dataset.TRAIN_MODE if label == 'train' else  Dataset.CALIBRATE_MODE if label == 'calibrate' else Dataset.EVAL_MODE
    ...
    def __getitem__(self, index: int):
        ...
        elif self._mode == Dataset.CALIBRATE_MODE:
            return sampling.create_train_sample(doc, 0, 0,
                                                self._max_span_size, len(self._rel_types))
        else:
