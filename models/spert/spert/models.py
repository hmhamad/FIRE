...
class SpERT(BertPreTrainedModel):
    ...
    def __init__(self, config: BertConfig, cls_token: int, num_relation_types: int, num_entity_types: int,
                 size_embedding: int, prop_drop: float,  max_pairs: int = 100):
        ...
        # layers
        self.rel_classifier = nn.Linear(config.hidden_size * 3 + size_embedding * 2 , num_relation_types)

        self.entity_classifier = nn.Linear(config.hidden_size * 2 + size_embedding, num_entity_types)
        self.size_embeddings = nn.Embedding(100, size_embedding)
  
        self.dropout = nn.Dropout(prop_drop)

        self._cls_token = cls_token
        self._relation_types = num_relation_types
        self._entity_types = num_entity_types
    ...
    def _forward_inference(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
        ...
        # classify entities
        size_embeddings = self.size_embeddings(entity_sizes)  # embed entity candidate sizes
        ent_logits, entity_spans_pool = self._classify_entities(encodings, h, entity_masks, size_embeddings)

        # ignore entity candidates that do not constitute an actual entity for relations (based on classifier)
        relations, rel_masks, rel_sample_masks = self._filter_spans(ent_logits, entity_spans,
        ...
        rel_clf = torch.zeros([batch_size, relations.shape[1], self._relation_types]).to(
            self.rel_classifier.weight.device)
        rel_logits = torch.zeros([batch_size, relations.shape[1], self._relation_types]).to(
            self.rel_classifier.weight.device)
        # obtain relation logits
        # chunk processing to reduce memory usage
        for i in range(0, relations.shape[1], self._max_pairs):
            ...
            rel_logits[:, i:i + self._max_pairs, :] = chunk_rel_logits

        rel_clf = rel_clf * rel_sample_masks  # mask
        rel_logits = rel_logits * rel_sample_masks  # mask

        # apply softmax with temperature
        entity_clf = torch.softmax(ent_logits, dim=2)


        return ent_logits, entity_clf, rel_clf, rel_logits, relations, rel_repr

    def _forward_ensemble_inference(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
                           entity_sizes: torch.tensor, entity_spans: torch.tensor, entity_sample_masks: torch.tensor):
        # get contextualized token embeddings from last transformer layer
        context_masks = context_masks.float()
        h = self.bert(input_ids=encodings, attention_mask=context_masks)['last_hidden_state']
        ...
        return ent_logits, entity_clf, rel_clf, relations
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings):
        ...
        #if logit_norm:
        #    entity_clf = entity_clf / (torch.norm(entity_clf,dim=-1) * logit_norm_temp).unsqueeze(-1)
