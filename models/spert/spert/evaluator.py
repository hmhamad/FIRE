...
from models.spert.spert import prediction
from models.spert.spert.entities import Document, Dataset, EntityType
from models.spert.spert.input_reader import BaseInputReader
from models.spert.spert.opt import jinja2
...
class Evaluator:
    ...
    def eval_batch(self,  batch_entity_logits: torch.tensor, batch_entity_clf: torch.tensor, batch_rel_logits: torch.tensor, batch_rel_clf: torch.tensor,
                   batch_rels: torch.tensor, batch: dict):
        batch_pred_entities, batch_pred_relations = prediction.convert_predictions(batch_entity_logits,batch_entity_clf,
                                                                                     batch_rel_logits, batch_rel_clf,
                                                                                     batch_rels, batch,
    ...
    def compute_scores(self):
        ...
        ner_eval, ner_per_type = self._score(gt, pred, print_results=True)
        ...
        rel_eval, rel_per_type = self._score(gt, pred, print_results=True)
        ...
        rel_nec_eval, rel_nec_per_type = self._score(gt, pred, print_results=True)

        return ner_eval, rel_eval, rel_nec_eval, rel_nec_per_type
    ...
    def _score(self, gt: List[List[Tuple]], pred: List[List[Tuple]], print_results: bool = False):
        ...
        metrics, per_type = self._compute_metrics(gt_flat, pred_flat, types, print_results)
    ...
    def _compute_metrics(self, gt_all, pred_all, types, print_results: bool = False):
        ...
        return [m * 100 for m in micro + macro], [(f1*100,t.identifier) for f1,t in zip(per_type[2],types)]
    ...
    def _print_results(self, per_type: List, micro: List, macro: List, types: List):
        ...
        sorted_idx = sorted(range(len(metrics_per_type)),reverse=True,key= lambda i: metrics_per_type.__getitem__(i)[3])
        sorted_metrics_per_type = [metrics_per_type[i] for i in sorted_idx]
        sorted_types = [list(types)[i] for i in sorted_idx]
        for m, t in zip(sorted_metrics_per_type, sorted_types):
