...
from models.spert.spert.entities import Dataset, EntityType, RelationType, Entity, Relation, Document
from models.spert.spert.opt import spacy
...
class BaseInputReader(ABC):
    def __init__(self, types_path: str, tokenizer: BertTokenizer, neg_entity_count: int = None,
        ...
        self.entity_types_tokenized = OrderedDict()
        ...
        self.entity_types_tokenized['None'] = tokenizer(none_entity_type.identifier.lower())
        ...
        for i, (key, v) in enumerate(types['entities'].items()):
            ...
            self.entity_types_tokenized[key] = tokenizer(entity_type.identifier.lower())
...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_dataset(self, dataset_path, dataset):
        ...
        for idx,document in enumerate(tqdm(documents, desc="Parse dataset '%s'" % dataset.label)):
...
class JsonPredictionInputReader(BaseInputReader):
    ...
    def api_read(self, sentence, dataset_label):
        dataset = Dataset(dataset_label, self._relation_types, self._entity_types, self._neg_entity_count,
                          self._neg_rel_count, self._max_span_size)
