...
def _add_common_args(arg_parser):
    arg_parser.add_argument('--config', default = 'models/spert/configs/temp.conf', type=str)
    arg_parser.add_argument('--device_id', type=int, default = 2)

    # Input
    arg_parser.add_argument('--types_path', type=str, help="Path to type specifications")

    # Preprocessing
    arg_parser.add_argument('--tokenizer_path', default = 'bert-base-cased', type=str, help="Path to tokenizer")
    ...
    arg_parser.add_argument('--sampling_processes', type=int, default=4, help="Number of sampling processes. 0 = no multiprocessing for sampling")

    # Model / Training / Evaluation
    arg_parser.add_argument('--model_path', type=str, default='bert-base-cased', help="Path to directory that contains model checkpoints")
    ...
    arg_parser.add_argument('--max_pairs', type=int, default=1000, help="Maximum entity pairs to process during training/evaluation")
    ...
    arg_parser.add_argument('--no_overlapping', action='store_true', default=False, help="If true, do not evaluate on overlapping entities " "and relations with overlapping entities")

    # Misc
    arg_parser.add_argument('--seed', type=int, default=2021, help="Seed")
...
def _add_logging_args(arg_parser):
    ...
    arg_parser.add_argument('--example_count', type=int, default=5, help="Count of evaluation example to store (if store_examples == True)")
...
def train_argparser():
    ...
    arg_parser.add_argument('--init_eval', action='store_true', default=False, help="If true, evaluate validation set before training")
    ...
    arg_parser.add_argument('--neg_entity_count', type=int, default=100, help="Number of negative entity samples per document (sentence)")
    arg_parser.add_argument('--neg_relation_count', type=int, default=100, help="Number of negative relation samples per document (sentence)")
    ...
    # Save Trained Model 
    arg_parser.add_argument('--save_model', action='store_true', default=False, help="If true, store model as pytorch_model.bin")
