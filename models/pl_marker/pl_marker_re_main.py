...
def evaluate(args, model, tokenizer, prefix="", data_file_path = "", save_embeddings = False):
    ...
    if args.do_predict:
        ...
        with open(os.path.join(args.output_dir, 'predictions.json'), 'w') as output_w:
...
def call_pl_marker_re(importargs=None):
    ...
    if args.do_test or args.do_predict:
        ...
        model.to(args.device)
        results = evaluate(args, model, tokenizer, prefix = 'test', data_file_path=args.data_file, save_embeddings = args.save_embeddings)
        print (results)

        if args.do_test:
            F1_path = os.path.join(args.output_dir, 'F1_test.csv')
